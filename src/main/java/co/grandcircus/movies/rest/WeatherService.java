...
public class WeatherService {
	...
	public Weather getCurrentWeatherAt(String lat, String lon) {
		...
		try (BufferedReader reader = HttpHelper.doGet(url)) { // try with
			...
			JsonObject time = root.getAsJsonObject().get("time").getAsJsonObject();
			JsonArray startPeriodName = time.get("startPeriodName").getAsJsonArray();
			String firstValue = startPeriodName.get(0).getAsString();
			...
			weather.setWeather(currentObservation.get("Weather").getAsString());
            weather.setFirstvalue(firstValue);
			return weather;
