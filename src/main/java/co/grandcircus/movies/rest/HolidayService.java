...
public class HolidayService {
	...
	public ArrayList<Holiday> getCurrentHolidayAt(String key, String country,Integer year,Integer month) {
		String url = "https://holidayapi.com/v1/holidays?key=" + key + "&country=" + country + "&year=" + year + "&month=" + month + "&FcstType=json";
		...
		try (BufferedReader reader = HttpHelper.doGet(url)) { // try with
			...
			//JsonObject response = root.getAsJsonObject().get("response").getAsJsonObject();
